#!/usr/bin/env bash
# run_clique_integration.sh - full pipeline: compile → pass → link → run
#
# Mirrors run_kcolor_integration.sh for clique-pass. Uses the reduced
# canonicalization pipeline (no loop-simplify/lcssa/indvars) confirmed
# correct in analysis/clique/clique.md §1c -- clique-pass matches a
# self-recursive shape, and those three passes actively break the match
# (see clique.md for the confirmed root cause: indvars hides the loop
# bound argument inside an llvm.smax call).
#
# Steps:
#   1. clang: C++ source → unoptimised LLVM IR
#   2. opt:   canonicalisation passes + clique-pass → transformed IR
#              (top-level maxCliques(0, ..., 0, ...) call replaced with
#              @clique_impl)
...
"$CLANG" -S -emit-llvm -O0 -fno-inline \
    ...
    -std=c++20 \
    "$SRC" -o "$BUILD/clique_e2e.ll" 2>&1
echo "      $BUILD/clique_e2e.ll"

# Step 2: canonicalization passes only (no clique-pass)
echo ""
echo "[2/5] opt: canonicalization passes → canon IR"
"$OPT" -passes="$OPT_PASSES" \
    "$BUILD/clique_e2e.ll" -S -o "$BUILD/clique_e2e_canon.ll" 2>&1
echo "      $BUILD/clique_e2e_canon.ll"

# Step 3: clique-pass on already-canonicalised IR
echo ""
echo "[3/5] opt: clique-pass → transformed IR"
"$OPT" -load-pass-plugin "$PLUGIN" \
    -passes=clique-pass \
    "$BUILD/clique_e2e_canon.ll" -S -o "$BUILD/clique_e2e_transformed.ll" 2>&1
echo "      $BUILD/clique_e2e_transformed.ll"

echo ""
if grep -q "call i32 @clique_impl" "$BUILD/clique_e2e_transformed.ll"; then
    echo "      ✓  @clique_impl injected — pass fired"
    echo ""
    echo "      Call site:"
    grep "clique_impl" "$BUILD/clique_e2e_transformed.ll" \
        | grep -v "^;" | grep -v "@\.str" | sed 's/^/        /'
else
    echo "      ✗  @clique_impl NOT found — pass did not fire"
    exit 1
fi
...
"$CLANG" -c "$BUILD/clique_e2e_transformed.ll" -o "$BUILD/clique_e2e.o" 2>&1
echo "      $BUILD/clique_e2e.o"
...
"$CLANG" \
    ...
    -o "$BUILD/clique_e2e" 2>&1
echo "      $BUILD/clique_e2e"
