#!/usr/bin/env bash
# run_tsp_integration.sh - full pipeline: compile → pass → link → run
#
# Mirrors run_integration.sh (MaxCut's e2e harness) for tsp-pass. Uses the
# same full canonicalization pipeline maxcut-pass does (including
# loop-simplify/lcssa/indvars) since tsp-pass matches an outer/inner loop
# nest, not a self-recursive shape -- see test/tsp/update.py's own
# OPT_PASSES, which this mirrors exactly.
#
# Steps:
#   1. clang: C++ source → unoptimised LLVM IR
#   2. opt:   canonicalisation passes + tsp-pass → transformed IR
#              (brute-force permutation loop replaced with @tsp_impl)
...
OPT_PASSES="sroa,mem2reg,loop-simplify,lcssa,indvars,simplifycfg,instcombine<no-verify-fixpoint>,simplifycfg,instcombine"

SRC="$SCRIPT_DIR/tsp_e2e.cpp"
...
"$CLANG" -S -emit-llvm -O0 -fno-inline \
    ...
    -std=c++20 \
    "$SRC" -o "$BUILD/tsp_e2e.ll" 2>&1
echo "      $BUILD/tsp_e2e.ll"

# Step 2: canonicalization passes only (no tsp-pass)
echo ""
echo "[2/5] opt: canonicalization passes → canon IR"
"$OPT" -passes="$OPT_PASSES" \
    "$BUILD/tsp_e2e.ll" -S -o "$BUILD/tsp_e2e_canon.ll" 2>&1
echo "      $BUILD/tsp_e2e_canon.ll"

# Step 3: tsp-pass on already-canonicalised IR
echo ""
echo "[3/5] opt: tsp-pass → transformed IR"
"$OPT" -load-pass-plugin "$PLUGIN" \
    -passes=tsp-pass \
    "$BUILD/tsp_e2e_canon.ll" -S -o "$BUILD/tsp_e2e_transformed.ll" 2>&1
echo "      $BUILD/tsp_e2e_transformed.ll"

echo ""
if grep -q "call i32 @tsp_impl" "$BUILD/tsp_e2e_transformed.ll"; then
    echo "      ✓  @tsp_impl injected — pass fired"
    echo ""
    echo "      Call site:"
    grep "tsp_impl" "$BUILD/tsp_e2e_transformed.ll" \
        | grep -v "^;" | grep -v "@\.str" | sed 's/^/        /'
else
    echo "      ✗  @tsp_impl NOT found — pass did not fire"
    exit 1
fi
...
"$CLANG" -c "$BUILD/tsp_e2e_transformed.ll" -o "$BUILD/tsp_e2e.o" 2>&1
echo "      $BUILD/tsp_e2e.o"
